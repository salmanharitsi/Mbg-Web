...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div
          className={`w-full max-w-6xl rounded-2xl transition-all duration-500 ${scrolled ? "glass-light bg-white/50 backdrop-blur-[4px]" : "glass"}`}
          style={scrolled ? { boxShadow: "0 8px 32px rgba(22,163,74,0.12)" } : {}}
        >
          <nav className="flex items-center justify-between px-5 py-3">
            ...
            <Link href="/" className="flex items-center gap-2.5 group">
              ...
              <div className="hidden sm:block">
                <p className={`font-display font-bold text-sm leading-tight ${scrolled ? "text-primary-800" : "text-white"}`}>
                  Badan Gizi Nasional
                </p>
                <p className={`text-[10px] font-body leading-none ${scrolled ? "text-primary-600" : "text-gray-400"}`}>
              ...
              {navLinks.map((link) => (
                <li key={link.href}>
                  <a
                    ...
                    style={{
                      color: scrolled
                        ? activeHash === link.href ? "#15803d" : "#166534"
                        : activeHash === link.href ? "#fff" : "rgba(255,255,255,0.8)",
                      backgroundColor: activeHash === link.href
                        ? scrolled ? "rgba(22,163,74,0.12)" : "rgba(255,255,255,0.1)"
                        : "transparent",
                    }}
                  >
                    {link.label}
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            style={{ backgroundColor: "rgba(2,26,12,0.8)", backdropFilter: "blur(4px)" }}
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            <div className="flex items-center justify-between p-5" style={{ borderBottom: "1px solid #dcfce7" }}>
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                  style={{ background: "linear-gradient(135deg, #16a34a, #14532d)" }}>
                  <span className="text-white font-display font-bold text-xs">BGN</span>
                </div>
                <span className="font-display font-bold text-sm" style={{ color: "#14532d" }}>
                  Badan Gizi Nasional
                </span>
              </div>
              <button onClick={() => setMenuOpen(false)} className="p-1.5 rounded-lg" style={{ color: "#15803d" }}>
                <X size={20} />
              </button>
            </div>
